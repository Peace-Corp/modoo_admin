...
export async function sendDesignProofEmail(params: DesignProofEmailParams): Promise<boolean> {
  ...
  const previewHtml = previewUrl
    ? `<div style="text-align:center;margin:20px 0;padding:20px;background:#f8f9fc;border-radius:12px;">
    ...
    <div style="font-family:'Apple SD Gothic Neo','Malgun Gothic',sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
      ...
      <div style="padding:32px 28px;">
        <div style="text-align:center;margin-bottom:24px;">
          <div style="display:inline-block;background:#7c3aed;color:#fff;padding:8px 20px;border-radius:20px;font-size:14px;font-weight:600;">시안 확인 요청</div>
        </div>
        ...
        <div style="text-align:center;margin:28px 0;">
          <a href="${confirmUrl}" style="display:inline-block;padding:16px 40px;background-color:#7c3aed;color:#ffffff;border-radius:10px;font-weight:bold;font-size:15px;text-decoration:none;">시안 확인하기</a>
        </div>
  ...
  try {
    return await sendGmailEmail({
      ...
      subject: `[모두의 유니폼] 시안이 준비되었습니다 - ${itemLabel} (${shortOrderId})`,
      text,
      html,
    });
  } catch (error) {
